...
(2)${\lambda}$是泊松分布所依赖的唯一参数。${\lambda}$值愈小，分布愈偏倚，随着${\lambda}$增大，分布趋于对称。 
(3)当${\lambda}$=20时分布泊松分布接近于正态分布；当${\lambda}$=50时，可以认为泊松分布呈正态分布。 在实际工作中，当${\lambda}$大于等于20时就可以用正态分布来近似地处理泊松分布的问题。 
